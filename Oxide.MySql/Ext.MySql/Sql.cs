...
namespace Oxide.Ext.MySql
{
	public class Sql
	{
		...
		{
			return new Sql.SqlJoinClause(this.Append(new Sql(string.Concat(joinType, table), new object[0])));
		}
		...
		public static string ProcessParams(string sql, object[] argsSrc, List<object> argsDest)
		{
			return Sql.RxParams.Replace(sql, (Match m) => {
				...
				if (!int.TryParse(str1, out num))
				{
					...
					if (!flag)
					{
						throw new ArgumentException(string.Format("Parameter '@{0}' specified but none of the passed arguments have a property with this name (in '{1}')", str1, sql));
					}
				}
				else
				{
