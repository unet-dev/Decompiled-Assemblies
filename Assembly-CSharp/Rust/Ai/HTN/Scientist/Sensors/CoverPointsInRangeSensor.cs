...
namespace Rust.Ai.HTN.Scientist.Sensors
{
	[Serializable]
	public class CoverPointsInRangeSensor : INpcSensor
	{
		...
		private bool _FindCoverPointsInVolume(Vector3 position, List<CoverPoint> coverPoints, ref CoverPointVolume volume, ref float nextTime, float time, AiLocationManager location, float maxDistanceToCoverSqr)
		{
			...
			if (time > nextTime)
			{
				...
				if ((volume == null || !volume.Contains(position)) && SingletonComponent<AiManager>.Instance != null && SingletonComponent<AiManager>.Instance.enabled && SingletonComponent<AiManager>.Instance.UseCover)
				{
		...
		{
			ScientistDomain aiDomain = npc.AiDomain as ScientistDomain;
