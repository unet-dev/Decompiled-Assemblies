...
namespace Rust.Ai.HTN.ScientistJunkpile
{
	...
	public class ScientistJunkpileContext : BaseNpcContext, IDisposable
	{
		...
		{
			int num = fact;
			if (ScientistJunkpileContext._worldStateChanges[num].Count <= 0)
			{
				return (byte)0;
			}
			return ScientistJunkpileContext._worldStateChanges[num].Peek().Value;
		}
		...
		public void PopFactChangeDuringPlanning(byte fact)
		{
			...
			if (ScientistJunkpileContext._worldStateChanges[num].Count > 0)
			{
				ScientistJunkpileContext._worldStateChanges[num].Pop();
			}
		}
		...
		{
			Stack<WorldStateInfo> worldStateInfos = ScientistJunkpileContext._worldStateChanges[fact];
		...
		public void SetFact(Facts fact, byte value, bool invokeChangedEvent = true, bool setAsDirty = true, bool checkValueDiff = true)
		{
			...
			{
				ScientistJunkpileContext.WorldStateChangedEvent onWorldStateChangedEvent = this.OnWorldStateChangedEvent;
