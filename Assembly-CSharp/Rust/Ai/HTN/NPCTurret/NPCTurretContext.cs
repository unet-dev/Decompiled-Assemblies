...
namespace Rust.Ai.HTN.NPCTurret
{
	...
	public class NPCTurretContext : BaseNpcContext, IDisposable
	{
		...
		public NPCTurretContext.WorldStateChangedEvent OnWorldStateChangedEvent;
		...
		public override Stack<WorldStateInfo>[] WorldStateChanges
		{
			get
			{
				return NPCTurretContext._worldStateChanges;
			}
		}
		...
		public NPCTurretContext(HTNPlayer body, NPCTurretDomain domain)
		{
			...
			if (this._worldState == null || (int)this._worldState.Length != length)
			{
				...
				if (NPCTurretContext._worldStateChanges == null)
				{
					NPCTurretContext._worldStateChanges = new Stack<WorldStateInfo>[length];
					for (int i = 0; i < length; i++)
					{
						NPCTurretContext._worldStateChanges[i] = new Stack<WorldStateInfo>(5);
					}
				}
			}
			...
			if (this.Memory == null || this.Memory.NPCTurretContext != this)
			{
				this.Memory = new NPCTurretMemory(this);
			}
		...
		public override Vector3 GetDirectionToMemoryOfPrimaryEnemyPlayerTarget()
		{
			...
			float bodyPosition = vector3.y - this.BodyPosition.y;
			Vector3 lastKnownPosition = (primaryKnownEnemyPlayer.LastKnownPosition + (primaryKnownEnemyPlayer.PlayerInfo.Player.transform.up * bodyPosition)) - vector3;
		...
		{
			int num = fact;
			if (NPCTurretContext._worldStateChanges[num].Count <= 0)
			{
				return (byte)0;
			}
			return NPCTurretContext._worldStateChanges[num].Peek().Value;
		}
		...
		public void PopFactChangeDuringPlanning(byte fact)
		{
			...
			if (NPCTurretContext._worldStateChanges[num].Count > 0)
			{
				NPCTurretContext._worldStateChanges[num].Pop();
			}
		}
		...
		{
			Stack<WorldStateInfo> worldStateInfos = NPCTurretContext._worldStateChanges[fact];
