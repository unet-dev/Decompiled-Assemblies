...
namespace Rust.Ai.HTN.NPCTurret
{
	...
	public class NPCTurretDefinition : BaseNpcDefinition
	{
		...
		private IEnumerator EquipWeapon(HTNPlayer target)
		{
			...
			yield return CoroutineEx.waitForSeconds(0.25f);
			NPCTurretDomain aiDomain = target.AiDomain as NPCTurretDomain;
			if (aiDomain)
			{
				if (item.info.category == ItemCategory.Weapon)
				{
					...
					{
						aiDomain.NPCTurretContext.SetFact(Facts.HeldItemType, ItemType.ProjectileWeapon, true, true, true);
						aiDomain.ReloadFirearm();
					}
					else if (heldEntity is BaseMelee)
					{
						aiDomain.NPCTurretContext.SetFact(Facts.HeldItemType, ItemType.MeleeWeapon, true, true, true);
					}
					else if (heldEntity is ThrownWeapon)
					{
						aiDomain.NPCTurretContext.SetFact(Facts.HeldItemType, ItemType.ThrowableWeapon, true, true, true);
					}
				}
				else if (item.info.category == ItemCategory.Medical)
				{
					aiDomain.NPCTurretContext.SetFact(Facts.HeldItemType, ItemType.HealingItem, true, true, true);
				}
				else if (item.info.category == ItemCategory.Tool)
				{
					aiDomain.NPCTurretContext.SetFact(Facts.HeldItemType, ItemType.LightSourceItem, true, true, true);
				}
			}
		}
		...
		public override BaseCorpse OnCreateCorpse(HTNPlayer target)
		{
			...
			{
				NPCPlayerCorpse navAgent = target.DropCorpse((string.IsNullOrEmpty(this.CorpsePath) ? "assets/prefabs/npc/scientist/scientist_corpse.prefab" : this.CorpsePath)) as NPCPlayerCorpse;
				if (navAgent)
				{
		...
		{
			NPCTurretDefinition nPCTurretDefinition = null;
			while (nPCTurretDefinition._isRadioEffectRunning && target != null && target.transform != null && !target.IsDestroyed && !target.IsDead())
			{
				if (nPCTurretDefinition.RadioEffect.isValid)
				{
					Effect.server.Run(nPCTurretDefinition.RadioEffect.resourcePath, target, StringPool.Get("head"), Vector3.zero, Vector3.zero, null, false);
				}
				float single = UnityEngine.Random.Range(nPCTurretDefinition.RadioEffectRepeatRange.x, nPCTurretDefinition.RadioEffectRepeatRange.y + 1f);
