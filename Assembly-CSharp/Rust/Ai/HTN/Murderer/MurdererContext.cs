...
{
	[Serializable]
	public class MurdererContext : BaseNpcContext, IDisposable
	{
		[ReadOnly]
		[SerializeField]
		...
		public MurdererContext.WorldStateChangedEvent OnWorldStateChangedEvent;
		...
		public override Stack<WorldStateInfo>[] WorldStateChanges
		{
			get
			{
				return MurdererContext._worldStateChanges;
			}
		}
		...
		{
			int num = fact;
			if (MurdererContext._worldStateChanges[num].Count <= 0)
			{
				return (byte)0;
			}
			return MurdererContext._worldStateChanges[num].Peek().Value;
		}
		...
		public void PopFactChangeDuringPlanning(byte fact)
		{
			...
			if (MurdererContext._worldStateChanges[num].Count > 0)
			{
				MurdererContext._worldStateChanges[num].Pop();
			}
		}
		...
		public void SetFact(Facts fact, byte value, bool invokeChangedEvent = true, bool setAsDirty = true, bool checkValueDiff = true)
		{
			...
			{
				MurdererContext.WorldStateChangedEvent onWorldStateChangedEvent = this.OnWorldStateChangedEvent;
