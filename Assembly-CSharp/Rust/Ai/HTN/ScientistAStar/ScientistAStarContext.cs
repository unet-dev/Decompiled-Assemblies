...
namespace Rust.Ai.HTN.ScientistAStar
{
	...
	public class ScientistAStarContext : BaseNpcContext, IDisposable
	{
		...
		public ScientistAStarContext.WorldStateChangedEvent OnWorldStateChangedEvent;
		...
		public Vector3 BodyLocalPosition
		{
			get
			{
				return this.Body.transform.localPosition;
			}
		}
		...
		public override Stack<WorldStateInfo>[] WorldStateChanges
		{
			get
			{
				return ScientistAStarContext._worldStateChanges;
			}
		}
		...
		{
			int num = fact;
			if (ScientistAStarContext._worldStateChanges[num].Count <= 0)
			{
				return (byte)0;
			}
			return ScientistAStarContext._worldStateChanges[num].Peek().Value;
		}
		...
		public void PopFactChangeDuringPlanning(byte fact)
		{
			...
			if (ScientistAStarContext._worldStateChanges[num].Count > 0)
			{
				ScientistAStarContext._worldStateChanges[num].Pop();
			}
		}
		...
		{
			Stack<WorldStateInfo> worldStateInfos = ScientistAStarContext._worldStateChanges[fact];
		...
		public void SetFact(Facts fact, byte value, bool invokeChangedEvent = true, bool setAsDirty = true, bool checkValueDiff = true)
		{
			...
			{
				ScientistAStarContext.WorldStateChangedEvent onWorldStateChangedEvent = this.OnWorldStateChangedEvent;
